...
class TelegramDownloadHelper(DownloadHelper):
    ...
    def __onDownloadProgress(self, current, total):
        if self.__is_cancelled:
            self.__onDownloadError('Cancelled by user! #CancelledByUser')
    ...
    def __download(self, message, path):
        ...
        if download is not None:
            ...
            if not self.__is_cancelled:
                self.__onDownloadError('Internal error occurred #InternalError')
    ...
    def add_download(self, message, path, filename):
        ...
        if media is not None:
            ...
            if download:
                ...
                self.__onDownloadError('File already being downloaded! #FileInQueue')
        else:
            self.__onDownloadError('No document in the replied message #NoDoc')
