...
class GoogleDriveHelper:
    ...
    def deletefile(self, link: str):
        try:
            ...
            msg = "Google drive ID could not be found in the provided link #NoID"
            return msg
        msg = ''
        try:
            res = self.__service.files().delete(fileId=file_id, supportsTeamDrives=IS_TEAM_DRIVE).execute()
            msg = "Successfully deleted #Deleted"
        except HttpError as err:
    ...
    def edit_telegraph(self):
        ...
        for content in self.telegraph_content :
            ...
            Telegraph(access_token=TELEGRAPH_TOKEN).edit_page(path = self.path[prev_page],
                                 title = '𝗠𝗜𝗥𝗥𝗢𝗥 BOTS by IshanAvasthi',
                                 author_name='👲 𝗨𝗽𝗹𝗼𝗮𝗱 𝗕𝘆',
                                 author_url='https://t.me/IshanAvasthi',
        ...
        if response["files"]:
            ...
            for content in self.telegraph_content :
                self.path.append(Telegraph(access_token=TELEGRAPH_TOKEN).create_page(
                                                        title='ᴅʀᴀɢᴏɴ ᴍɪʀʀᴏʀ ᴢᴏɴᴇ By - ɪꜱʜΔɴ ΔᴠΔꜱᴛʜɪ',
                                                        author_name='👲 𝐔𝐩𝐥𝐨𝐚𝐝𝐞𝐝 𝐁𝐲',
                                                        author_url='https://t.me/IshanAvasthi',
                                                        html_content=content
                                                        )['path'])
            ...
            msg = f"<b>🔎 Search Results For <i>{fileName}</i></b> \n<b>📚 Found {len(response['files'])} results #SearchResult @DragonMirror</b>"
            buttons = button_build.ButtonMaker()   
