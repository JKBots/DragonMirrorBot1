...
def cancel_mirror(update, context):
    ...
    if len(args) > 1:
        ...
        if not dl:
            sendMessage(f"GID: <code>{gid}</code> not found. Copy GID and send with cancel command. Copy corrent GID from /status", context.bot, update)
            return
        ...
        if mirror_message is None or mirror_message.message_id not in keys:
            if BotCommands.MirrorCommand in update.message.text or \
               ...
               BotCommands.UnzipMirrorCommand in update.message.text:
                ...
                msg = "Please reply to the /mirror6 message which was used to start the download or /cancel6 gid to cancel it!"
                sendMessage(msg, context.bot, update)
                return
